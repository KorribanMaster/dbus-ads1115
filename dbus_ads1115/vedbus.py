...
class VeDbusService(object):
    ...
    def __init__(self, servicename, bus=None):
        ...
        self._dbusconn = bus or (dbus.SessionBus()
        ...
        self._dbusname = dbus.service.BusName(servicename,
                                              self._dbusconn,
                                              do_not_queue=True)
    ...
    def __del__(self):
        ...
        if self._dbusname:
            ...
            )  # Forces call to self._bus.release_name(self._name), see source code
        self._dbusname = None
    ...
    def add_path(self,
        ...
        item = VeDbusItemExport(self._dbusconn,
                                path,
                                value,
                                description,
                                writeable,
        ...
        for i in range(2, len(spl)):
            ...
            if subPath not in self._dbusnodes and subPath not in self._dbusobjects:
                self._dbusnodes[subPath] = VeDbusTreeExport(
                    self._dbusconn, subPath, self)
        self._dbusobjects[path] = item
        logging.debug('added %s with start value %s. Writeable is %s' %
    ...
    def __delitem__(self, path):
        self._dbusobjects[path].__del__(
        )  # Invalidates and then removes the object path
    ...
    def __enter__(self):
        ...
        self._ratelimiters.append(context)
        return context
...
class VeDbusRootTracker(object):
    ...
    def __init__(self, bus, serviceName):
        ...
        self._match = bus.get_object(
            serviceName, '/', introspect=False).connect_to_signal(
    ...
    def _items_changed_handler(self, items):
        ...
        for path, changes in items.items():
            ...
            try:
                ...
                t = str(dbus_ads1115.ve_utils.unwrap_dbus_value(v))
...
class VeDbusItemImport(object):
    ...
    def __init__(self,
        ...
        assert eventCallback is None or createsignal is True
        if createsignal is True:
        ...
        try:
            ...
            self._cachedvalue = dbus_ads1115.ve_utils.unwrap_dbus_value(v)
    ...
    def _refreshcachedvalue(self):
        self._cachedvalue = dbus_ads1115.ve_utils.unwrap_dbus_value(
            self._proxy.GetValue())
    ...
    def set_value(self, newvalue):
        ...
        r = self._proxy.SetValue(
            dbus_ads1115.ve_utils.wrap_dbus_value(newvalue))
    ...
    def _properties_changed_handler(self, changes):
        ...
        if "Value" in changes:
            changes['Value'] = dbus_ads1115.ve_utils.unwrap_dbus_value(
                changes['Value'])
            ...
            if self._eventCallback:
                ...
                try:
                    ...
                    os._exit(
                        1
                    )  # sys.exit() is not used, since that also throws an exception
...
class VeDbusTreeExport(dbus.service.Object):
    ...
    def _get_value_handler(self, path, get_text=False):
        ...
        for p, item in self._service._dbusobjects.items():
            if p.startswith(px):
                v = item.GetText(
                ) if get_text else dbus_ads1115.ve_utils.wrap_dbus_value(
...
class VeDbusRootExport(VeDbusTreeExport):
    ...
    @dbus.service.method('com.victronenergy.BusItem',
                         out_signature='a{sa{sv}}')
    def GetItems(self):
        return {
            path: {
                'Value':
                dbus_ads1115.ve_utils.wrap_dbus_value(item.local_get_value()),
                'Text':
                item.GetText()
            }
...
class VeDbusItemExport(dbus.service.Object):
    ...
    def _local_set_value(self, newvalue):
        ...
        return {
            'Value': dbus_ads1115.ve_utils.wrap_dbus_value(newvalue),
            'Text': self.GetText()
        }
    ...
    def SetValue(self, newvalue):
        ...
        newvalue = dbus_ads1115.ve_utils.unwrap_dbus_value(newvalue)
        ...
        if (self._onchangecallback is None or (self._onchangecallback is not None and self._onchangecallback(self.__dbus_object_path__, newvalue))):
    ...
    def GetValue(self):
        ...
        """
        return dbus_ads1115.ve_utils.wrap_dbus_value(self._value)
