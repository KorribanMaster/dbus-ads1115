...
class SettingsDevice(object):
    ...
    def addSettings(self, settings):
        for setting, options in settings.items():
            ...
            busitem = self.addSetting(options[PATH],
                                      options[VALUE],
                                      options[MINIMUM],
                                      options[MAXIMUM],
                                      silent,
                                      callback=partial(
                                          self.handleChangedSetting, setting))
    ...
    def addSetting(self, path, value, _min, _max, silent=False, callback=None):
        busitem = dbus_ads1115.vedbus.VeDbusItemImport(self._bus,
                                                       self._dbus_name, path,
                                                       callback)
        if busitem.exists and (value, _min, _max,
                               silent) == busitem._proxy.GetAttributes():
            ...
            settings_item = dbus_ads1115.vedbus.VeDbusItemImport(
                self._bus, self._dbus_name, '/Settings', createsignal=False)
            ...
            busitem = dbus_ads1115.vedbus.VeDbusItemImport(
                self._bus, self._dbus_name, path, callback)
    ...
    def setDefault(self, path):
        item = dbus_ads1115.vedbus.VeDbusItemImport(self._bus,
                                                    self._dbus_name,
                                                    path,
                                                    createsignal=False)
        item.set_default()
